...
func StartHttp() {
	for _, bind := range Conf.WebsocketBind {
		...
		go websocketListen(bind, false)
	}

	for _, sslbind := range Conf.WebsocketSslBind {
		glog.Infof("start websocket listen addr:\"%s\"", sslbind)
		go websocketListen(sslbind, true)
	}
}

func websocketListen(bind string, ssl bool) {
	...
	if Conf.TCPKeepalive {
		...
		if ssl && Conf.WebsocketSslCrt != "" && Conf.WebsocketSslKey != "" {
			if err := http.ListenAndServeTLS(bind, Conf.WebsocketSslCrt, Conf.WebsocketSslKey, httpServeMux); err != nil {
...
func SubscribeHandle(ws *websocket.Conn) {
	...
	for {
		...
		if reply == Heartbeat {
			...
			glog.Errorf("<%s> user_key:\"%s\" receive message: %s", addr, key, reply)
